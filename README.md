...
### 功能说明
#### 代码说明
本项目基于SpringBoot框架，代码各层之间采用了松耦合的方式，使用大量dto和dao，以增强项目的拓展性。由于后端功能过于繁多，具体可到service层各接口查看。总得来说service涵盖了所有数据库的更删改查（有需要的），以及各个数据库间的联合查询，数据处理方面主要由service层完成逻辑处理，utils包内各类完成数学运算。AOP是作为新数据插入本地数据库后推送websocket消息使用的。
controller层和vo层有些混乱，因为是基于前端需求写的，由于需求不断提出和修改代码间逻辑结构不够明晰，但是具体说明可在代码内注释查看。
**现阶段未完成的任务在todo中，在写之前请务必查看**
#### 数据库说明
本项目数据库大概分为两大部分，一个是船只相关，包括船只本体、船只轨迹、船只种类、船只航线、航线各段五张表；一个是管道相关，包括管道本体、管道各段、管道传感器、管道传感器历史纪录、管道传感器种类五张表。两大部分基本独立，各部份内部由外键连接。

### 后端下一步规划
1. 区域查找：在指定地点为圆心的任意半径内查找船只和轨迹，现在使用的是like方式的数据库匹配查找，查找范围大且浪费资源，建议尝试使用正则表达式或者包含相邻经纬度内查找
2. 可疑船只分析：目前可以通过输入时间段查找穿越告警管道的船只和穿越的次数，更进一步的需求是计算其穿越的概率。
3. 危险船只预测：通过历史大数据预测未来船只将会出现的地点，进而计算出船会对管道造成破坏的概率
4. 管道未来故障预测：拟合管道历史故障曲线，预估管道出故障的时间点。比如根据管压传感器数据预估管道的寿命。
